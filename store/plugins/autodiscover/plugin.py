    ...
    *,
    ...
    async def on_host_scanned(ip: str, open_ports: list[dict[str, Any]]) -> None:
        ...
        if open_ports or mapped_items:
            ...
            logger.info(
                "Autodiscover host discovered ip=%s open_ports=%s dashboard_items=%s is_new=%s important=%s",
                ip,
                len(merged_open_ports),
                len(mapped_items),
                is_new,
                important,
            )
        ...
        if scanned_hosts_done % 8 == 0 or scanned_hosts_done == len(host_ips):
            await _emit_progress(
                ...
                "scan_progress",
                {
                    ...
                    "discovered_hosts": len(progress_hosts),
                    "discovered_services": services_found_total,
                    "chunk_tasks_done": chunk_tasks_done,
                    "chunk_tasks_total": chunk_tasks_total,
                },
            )

    async def on_chunk_scanned(ip: str, start_port: int, end_port: int, open_count: int) -> None:
        nonlocal chunk_tasks_done
        chunk_tasks_done += 1
        if open_count > 0:
            logger.info(
                "Autodiscover chunk scanned ip=%s range=%s-%s open_ports=%s task=%s/%s",
                ip,
                start_port,
                end_port,
                open_count,
        ...
        if chunk_tasks_done % max(1, len(host_ips)) == 0 or chunk_tasks_done == chunk_tasks_total:
            await _emit_progress(
                progress_callback,
                "scan_progress",
                {
                    "stage": "port-scan",
                    "ip": ip,
                    "range": {"from": start_port, "to": end_port},
    ...
    open_ports_map = await scan_open_ports(
        ...
        on_chunk_scanned=on_chunk_scanned,
    )
    await _emit_progress(
        progress_callback,
        "scan_stage",
    ...
    result_payload = {
        ...
        "scanned_ports": len(host_ips) * len(request.ports),
        "chunk_tasks_total": chunk_tasks_total,
        "chunk_tasks_completed": chunk_tasks_done,
        "scanned_cidrs": scanned_cidrs,
        "hosts": hosts,
        "source_file": str(request.result_file) if request.result_file else None,
    }
