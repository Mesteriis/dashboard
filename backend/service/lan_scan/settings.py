...
def _default_result_file() -> Path:
    return _default_base_dir().parent / "data" / "lan_scan_result.json"
...
class LanScanSettings(BaseSettings):
    ...
    ports: Annotated[tuple[int, ...], NoDecode] = Field(
        default_factory=lambda: SAFE_DEFAULT_SCAN_PORTS,
        validation_alias="LAN_SCAN_PORTS",
    )
    cidrs: Annotated[tuple[str, ...], NoDecode] = Field(
        default_factory=tuple,
        validation_alias=AliasChoices("LAN_SCAN_CIDRS", "LAN_SCAN_NETWORKS"),
    )
    result_file: Path = Field(
        default_factory=_default_result_file,
