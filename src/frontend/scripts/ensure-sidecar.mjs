...
const sidecarPath = path.join(frontendDir, 'src-tauri', 'binaries', 'oko-backend-aarch64-apple-darwin')
const buildScript = path.join(srcDir, 'desktop', 'build_sidecar_macos_arm64.sh')
...
if (!hasUsableSidecar(sidecarPath)) {
  throw new Error(`Sidecar build completed without output binary: ${sidecarPath}`)
}
