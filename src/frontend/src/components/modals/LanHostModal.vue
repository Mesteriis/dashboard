<template>
  ...
  >
    <header class="lan-host-modal-header">
      <h3>
        {{
          lanHostModal.host?.hostname ||
          lanHostModal.host?.ip ||
          "Устройство LAN"
        }}
      </h3>
      <button class="ghost" type="button" @click="closeLanHostModal">
        Закрыть
      </button>
    </header>
    ...
    <div v-if="lanHostModal.host" class="lan-host-modal-body">
      ...
      <section class="lan-host-section">
        ...
        <div v-if="lanHostModal.host.open_ports?.length" class="lan-port-chips">
          <span
            v-for="port in lanHostModal.host.open_ports"
            :key="port.port"
            class="lan-port-chip"
          >
            {{ port.port }}
      ...
      <section class="lan-host-section">
        ...
        <div
          v-if="lanHostModal.host.http_services?.length"
          class="lan-http-grid"
        >
          <article
          ...
          >
            <header class="lan-http-head">
              <strong
                >{{ endpoint.scheme.toUpperCase() }}:{{ endpoint.port }}</strong
              >
            ...
            <p class="lan-http-title">
              {{ endpoint.title || "Title не найден" }}
            </p>
            <p class="lan-http-description">
              {{ endpoint.description || "Описание не найдено" }}
            </p>
            <p v-if="endpoint.server" class="lan-http-server">
              Server: {{ endpoint.server }}
            </p>
            <p v-if="endpoint.error" class="widget-error">
              {{ endpoint.error }}
            </p>
          </article>
        </div>
      ...
      <section class="lan-host-section">
        ...
        <div
          v-if="lanHostModal.host.dashboard_items?.length"
          class="lan-service-links"
        >
          <a
