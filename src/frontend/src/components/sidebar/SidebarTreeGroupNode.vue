<template>
  ...
  >
    <div class="tree-node-row">
      <button class="tree-node tree-group" :class="{ active: isGroupSelected(group.key) }" type="button" @click="toggleGroupNode(group.key)">
    ...
    <ul v-show="isGroupExpanded(group.key)" class="tree-subgroups">
      <SidebarTreeSubgroupNode v-for="subgroup in group.subgroups" :key="subgroup.id" :group="group" :subgroup="subgroup" />
    </ul>
  </li>
</template>
