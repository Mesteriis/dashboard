...
export function createBrandIcon(icon: SimpleIcon): Component {
  const brandColor = `#${icon.hex}`;
  return (_props: unknown, { attrs }: IconRenderContext): VNode =>
    h(
      "svg",
      {
        ...attrs,
...
export const BRAND_ICON_BY_KEY: Record<string, Component> = {
