...
const emit = defineEmits(["hover-agent", "select-agent", "layout-update"]);
...
let activityBoostByAgentId = new Map();
let lastHoverAgentId = "";
let lastRoleSignature = "";
let roleSwapTransition = null;
let lastLayoutEmitMs = 0;
let linkCurveCache = new Map();
const avatarImageByAgentId = new Map();
...
function cloneLayoutMap(layoutMap) {
  const clone = new Map();
...
function targetStarCount(width, height) {
...
function resolveAvatarCandidates(rawSrc) {
...
function resolveAnimatedLayout(targetLayoutById, nowMs) {
...
function drawBackground(ctx, nowMs, width, height) {
...
function resolveCommunicationBoostMap(nowMs) {
...
function resolveEffectiveNodeRadius(node) {
  const boost = Number(activityBoostByAgentId.get(node?.id) || 0);
  return Math.max(0, Number(node?.radius || 0)) * (1 + boost);
}

function sampleQuadraticPoint(sourceNode, controlPoint, targetNode, t) {
  const oneMinusT = 1 - t;
  const x =
...
function drawPrimaryTransition(ctx, nowMs) {
...
function drawAgentNode(ctx, agent, node) {
...
function emitLayoutUpdate(nowMs) {
