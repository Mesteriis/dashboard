...
type ProjectionMode = "sphere" | "ring2d";
...
interface ParticleMetaEntry {
  ...
  clusterStickiness: number;
  laneSpread: number;
  lanePhase: number;
  avatarAngle: number;
  avatarSpeed: number;
  avatarTilt: number;
  avatarTiltDrift: number;
  avatarOrbitRadius: number;
  avatarJitter: number;
  maskWeight: number;
  prevX: number;
  prevY: number;
...
interface RebuildParticlesInput {
...
let avatarSpriteMaterial: THREE.SpriteMaterial | null = null;
...
let flowLineColorsBuffer: Float32Array | null = null;
let flowLineMeta: FlowLineMetaEntry[] = [];
let streamDefs: StreamDef[] = [];
...
function resolveStreamCount(rawCount: unknown, fallbackSeed: number): number {
...
function buildStreamDefs(
  ...
  const defs: StreamDef[] = new Array(streamCount);
...
function resolveTransformTarget(mode: unknown): number {
  return String(mode || "").toLowerCase() === "avatar" ? 0 : 1;
}

function resolveProjectionMode(value: unknown): ProjectionMode {
...
function resolveLowercaseCandidate(rawSrc: unknown): string {
...
function swapImageExtension(rawSrc: unknown, nextExtension: string): string {
  const src = String(rawSrc || "").trim();
  if (!src) return "";
...
function resolveImageFallbackCandidates(rawSrc: unknown): string[] {
...
function buildColorSampler(image: HTMLImageElement): ColorSampler | null {
...
function buildAvatarTextureFromSampler(
...
function getPointSpriteTexture(): THREE.CanvasTexture | null {
...
function rebuildParticlesFromImage({
  avatarImage,
  maskImage = null,
  sourceKey = "",
}: RebuildParticlesInput): void {
...
async function rebuildFromProps(): Promise<void> {
...
function attachResizeHandling(): void {
...
function detachResizeHandling(): void {
  if (resizeObserver) {
...
function attachVisibilityHandling(): void {
  const host = hostRef.value;
...
function detachVisibilityHandling(): void {
