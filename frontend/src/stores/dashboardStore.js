...
export function useDashboardStore() {
  ...
  const LAN_STREAM_RECONNECT_MS = 3000;
  ...
  const IMPORTANT_LAN_EVENT_PORTS = new Set([
    22,
    445,
    2375,
    3306,
  ...
  const lanScanLastActionAt = ref("");
  const lanScanLastActionMessage = ref("");
  const lanEventPopup = reactive({
    open: false,
    title: "",
    message: "",
    details: "",
  ...
  let lanScanStream = null;
  let lanScanStreamReconnectTimer = 0;
  let lanEventPopupTimer = 0;
  ...
  function markLanScanAction(message) {
    lanScanLastActionAt.value = new Date().toISOString();
    lanScanLastActionMessage.value = String(message || "").trim();
  }

  function closeLanEventPopup() {
    lanEventPopup.open = false;
    lanEventPopup.title = "";
    lanEventPopup.message = "";
    lanEventPopup.details = "";
    lanEventPopup.important = false;
    if (lanEventPopupTimer) {
      window.clearTimeout(lanEventPopupTimer);
      lanEventPopupTimer = 0;
    }
  }

  function showLanEventPopup({ title, message, details = "", important = false }) {
    lanEventPopup.open = true;
    lanEventPopup.title = String(title || "").trim() || "Событие LAN";
    lanEventPopup.message = String(message || "").trim();
    lanEventPopup.details = String(details || "").trim();
    lanEventPopup.important = Boolean(important);

    if (lanEventPopupTimer) {
      window.clearTimeout(lanEventPopupTimer);
    }
    lanEventPopupTimer = window.setTimeout(() => {
      lanEventPopupTimer = 0;
      closeLanEventPopup();
    }, important ? 8500 : 5000);
  }

  function normalizeLanHostPayload(host) {
    ...
    return {
      ip,
      hostname:
        typeof host.hostname === "string" && host.hostname.trim()
          ? host.hostname.trim()
          : null,
      mac_address:
        typeof host.mac_address === "string" && host.mac_address.trim()
          ? host.mac_address.trim()
          : null,
      mac_vendor:
        typeof host.mac_vendor === "string" && host.mac_vendor.trim()
          ? host.mac_vendor.trim()
          : null,
      device_type:
        typeof host.device_type === "string" && host.device_type.trim()
          ? host.device_type.trim()
          : null,
      open_ports: Array.isArray(host.open_ports)
        ? host.open_ports
            ...
            .filter(Boolean)
        : [],
      http_services: Array.isArray(host.http_services) ? host.http_services : [],
      dashboard_items: Array.isArray(host.dashboard_items)
        ? host.dashboard_items
        : [],
    };
  }

  function importantLanHost(host) {
    if (!host) return false;
    if ((host.dashboard_items || []).length) return true;
    return (host.open_ports || []).some((entry) =>
      IMPORTANT_LAN_EVENT_PORTS.has(Number(entry?.port || 0)),
    );
  }

  function ensureLanResultState() {
    if (!lanScanState.value || typeof lanScanState.value !== "object") {
      lanScanState.value = {
        enabled: true,
        scheduler: "asyncio",
        interval_sec: 1020,
    ...
    if (!lanScanState.value.result || typeof lanScanState.value.result !== "object") {
      lanScanState.value.result = {
    ...
    if (!Array.isArray(lanScanState.value.result.hosts)) {
      lanScanState.value.result.hosts = [];
    }

    return lanScanState.value.result;
  }

  function upsertLanHost(host) {
    const normalized = normalizeLanHostPayload(host);
    if (!normalized) return { isNew: false, host: null };

    const result = ensureLanResultState();
    const existingIndex = result.hosts.findIndex(
      (entry) => String(entry?.ip || "").trim() === normalized.ip,
    );
    const isNew = existingIndex < 0;
    if (isNew) {
      result.hosts.push(normalized);
    } else {
      result.hosts.splice(existingIndex, 1, normalized);
    }
    result.hosts.sort((left, right) =>
      String(left?.ip || "").localeCompare(String(right?.ip || ""), "en", {
        numeric: true,
      }),
    );
    return { isNew, host: normalized };
  }

  function applyLanStreamEvent(payload) {
    if (!payload || typeof payload !== "object") return;

    const streamType = String(payload.type || "").trim();
    const stateFromEvent =
      payload.state && typeof payload.state === "object" ? payload.state : null;
    if (stateFromEvent) {
      lanScanState.value = stateFromEvent;
    }

    let popupCandidate = null;
    if (payload.host && typeof payload.host === "object") {
      const upsert = upsertLanHost(payload.host);
      const important = Boolean(payload.important || importantLanHost(upsert.host));
      const eventIsNew = Boolean(payload.is_new || upsert.isNew);
      if ((eventIsNew || important) && upsert.host) {
        popupCandidate = {
          title: important ? "Важное LAN-событие" : "Новый хост в сети",
          message: upsert.host.hostname
            ? `${upsert.host.hostname} (${upsert.host.ip})`
            : upsert.host.ip,
          details: (upsert.host.open_ports || [])
            .slice(0, 6)
            .map((entry) =>
    ...
    const streamMessage = String(payload.message || "").trim();
    if (streamMessage) {
      markLanScanAction(streamMessage);
    }

    if (streamType === "scan_failed" && streamMessage) {
      lanScanError.value = streamMessage;
    }

    if (popupCandidate && isLanPage.value) {
      showLanEventPopup(popupCandidate);
    }
  }
  ...
  function stopLanScanStreaming() {
    if (lanScanStreamReconnectTimer) {
      window.clearTimeout(lanScanStreamReconnectTimer);
      lanScanStreamReconnectTimer = 0;
    }
    if (lanScanStream) {
      lanScanStream.close();
      lanScanStream = null;
  ...
  function startLanScanStreaming() {
    stopLanScanStreaming();
    const stream = createLanScanStream();
    if (!stream) return false;
    lanScanStream = stream;

    const handleEvent = (event) => {
      if (!event?.data) return;
      try {
        const payload = JSON.parse(event.data);
        applyLanStreamEvent(payload);
      } catch {
        // ignore malformed stream event payload
      }
    };

    stream.addEventListener("snapshot", handleEvent);
    stream.addEventListener("scan_started", handleEvent);
    stream.addEventListener("scan_queued", handleEvent);
    stream.addEventListener("scan_progress", handleEvent);
    stream.addEventListener("host_found", handleEvent);
    stream.addEventListener("scan_completed", handleEvent);
    stream.addEventListener("scan_failed", handleEvent);
    stream.onmessage = handleEvent;
    stream.onerror = () => {
      if (lanScanStream !== stream) return;
      stream.close();
      lanScanStream = null;

      if (!isDocumentVisible.value || !isLanPage.value) return;
      lanScanStreamReconnectTimer = window.setTimeout(() => {
        lanScanStreamReconnectTimer = 0;
        const connected = startLanScanStreaming();
        if (!connected) {
          refreshLanScanState({ silent: true });
          startLanScanPolling();
        }
      }, LAN_STREAM_RECONNECT_MS);
    };
    return true;
  }
  ...
  async function runLanScanNow() {
    if (lanScanActionBusy.value) {
      const message = "Запрос уже отправлен, дождитесь ответа backend.";
      lanScanError.value = message;
      markLanScanAction(message);
      return;
    }
    lanScanActionBusy.value = true;
    const isRunning = Boolean(lanScanState.value?.running);
    if (isRunning) {
      markLanScanAction(
        "Сканирование уже выполняется, отправляем запрос на постановку в очередь...",
      );
    } else {
      markLanScanAction("Отправляем запрос на запуск сканирования...");
    }
    try {
      ...
      if (payload?.accepted) {
        lanScanError.value = "";
        markLanScanAction("Backend принял запрос: сканирование запущено.");
      } else if (payload?.state?.queued) {
        lanScanError.value = "";
        markLanScanAction("Backend поставил запуск в очередь.");
      } else {
        const message = payload?.message || "Сканирование не запущено.";
        lanScanError.value = message;
        markLanScanAction(message);
      }
    } catch (error) {
      const message = error?.message || "Не удалось запустить сканирование LAN";
      lanScanError.value = message;
      markLanScanAction(message);
    } finally {
