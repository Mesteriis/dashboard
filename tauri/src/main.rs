...
const DEFAULT_REMOTE_BASE_URL: &str = "http://127.0.0.1:8000";
...
fn resolve_default_dashboard_template(app: &AppHandle) -> Option<PathBuf> {
  ...
  let candidates = [
    ...
    cwd.join("../dashboard.yaml"),
    cwd.join("dashboard.yaml"),
    cwd.join("../../dashboard.default.yaml"),
    cwd.join("../dashboard.default.yaml"),
    cwd.join("dashboard.default.yaml"),
  ];
...
fn resolve_dev_backend_script() -> Option<PathBuf> {
  let cwd = std::env::current_dir().ok()?;
  let candidates = [
    cwd.join("scripts/backend_sidecar.py"),
    cwd.join("../scripts/backend_sidecar.py"),
    cwd.join("../../scripts/backend_sidecar.py"),
  ];
...
fn spawn_embedded_backend(app: &AppHandle) -> Result<EmbeddedBackend, String> {
  ...
  let (mut command, command_hint) = if let Some(binary) = resolve_embedded_backend_binary(app) {
    ...
    return Err(
      "Embedded backend binary not found. Build sidecar and place it into tauri/binaries".to_string(),
    );
  };
...
mod tests {
  ...
  fn command_hint_matches_uses_basename() {
    let command_line = "python3 /tmp/project/scripts/backend_sidecar.py --host 127.0.0.1 --port 5000";
    assert!(command_hint_matches(command_line, Some("/Users/test/scripts/backend_sidecar.py")));
  }

  #[test]
  fn embedded_backend_command_accepts_known_binary() {
    let command_line =
      "/Applications/Oko.app/Contents/Resources/binaries/oko-backend-aarch64-apple-darwin --host 127.0.0.1 --port 8000";
  ...
  fn embedded_backend_command_rejects_hint_mismatch() {
    let command_line = "python3 /tmp/backend_sidecar.py --host 127.0.0.1 --port 8000";
